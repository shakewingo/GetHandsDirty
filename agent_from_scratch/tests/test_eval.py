...
from agent_from_scratch.evals.verify import metrics, summarize
...
class ReadEvaluationTests(unittest.TestCase):
    def test_summary_preserves_unknown_compaction_counts_in_legacy_and_mixed_rows(self):
        legacy = {'family': 'history', 'passed': True, 'invalid_calls': 0, 'parse_errors': 0,
                  'unexpected_changes': [], 'model_requests': 1, 'elapsed_seconds': 1.0,
                  'usage': {'prompt_tokens': 100, 'completion_tokens': 10, 'total_tokens': 110}}
        modern = {**legacy, 'compact_requests': 2, 'compactions_applied': 1}
        for rows, expected in (([legacy], (None, None)), ([legacy, modern], (None, None)),
                               ([modern, modern], (4, 2)),
                               ([{**modern, 'compact_requests': None}], (None, 1))):
            with self.subTest(expected=expected):
                report = summarize(rows)
                self.assertEqual((report['compact_requests'], report['compactions_applied']), expected)

    def test_review_cli_exports_legacy_results_without_model_calls(self):
        from agent_from_scratch.evals.run import main
        legacy = {'id': 'old', 'family': 'history', 'passed': True, 'invalid_calls': 0,
                  'parse_errors': 0, 'unexpected_changes': [], 'model_requests': 1,
                  'elapsed_seconds': 1.0, 'usage': dict.fromkeys(
                      ('prompt_tokens', 'completion_tokens', 'total_tokens'), None)}
        (self.workspace/'results.json').write_text(json.dumps([legacy]))
        review = self.workspace/'annotations.json'
        review.write_text(json.dumps({'old': {'false_completion': False, 'review_note': 'Verified evidence.'}}))
        with patch('sys.argv', ['eval', '--output', str(self.workspace), '--review', str(review)]), \
                patch('agent_from_scratch.evals.run.LLM', side_effect=AssertionError('No model during review')):
            main()
        report = json.loads((self.workspace/'summary-reviewed.json').read_text())
        self.assertEqual(report['passed'], 1)
        self.assertEqual(report['false_completion']['reviewed'], 1)
        self.assertIsNone(report['compact_requests'])
        self.assertIsNone(report['compactions_applied'])
        self.assertEqual(json.loads((self.workspace/'results.json').read_text()), [legacy])
