...
def get_tokenizer(config, ds, lang):
    tk_path = Path(config['tokenizer_file'].format(lang))
    if not Path.exists(tk_path):
...
def get_ds(config):
    ...
    # split 90% for training and 10% for val
...
def greedy_decode(model, src, src_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    """
    Allow reuse encoder output to translate a whole sentence in decoding part until finding EOS
    """
    ...
    # initialize the decoder input w sos token
    ...
    while True:
        ...
        if next_word == eos_idx:
            break
    return decoder_input.squeeze(0) # TODO: why suqeenze(0)? unchanged?
