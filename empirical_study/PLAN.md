...
- [ ] Run unchanged 17-task regression panel; record all outcomes, not an expected score.
- [ ] Commit protocol and baseline report.
...
- [ ] Write tests for known-good and wrong/incomplete artifact outcomes, no-op
  mutations, unknown usage and cost aggregation. Run to establish missing API failure.
- [ ] Implement `make_case(case_id, workspace)`, `score_case(case, result, workspace,
  before)`, `summarize(records)` and a fresh-output-only CLI.
- [ ] Freeze task selection, source/fixture/schema/settings hashes; preserve run traces.
- [ ] Run focused tests then full suite; save baseline general-tool model panel.
- [ ] Commit runner, reference tests, stage report and compact measurements.
...
- [ ] Write failing tests for raw preservation, old bulky body elision, metadata,
  unchanged recent/unsent/error observations, and measured no-op rejection.
- [ ] Implement bounded view replacements plus a soft-threshold prepass. Preserve
  replacements when testing summary candidates, but summarize original evidence.
- [ ] Check existing summary behavior and all context/batch tests.
- [ ] Record a matched model pilot and activation counts; commit report and code.

## Stage 03: planning

Files: `planning.py`, `agent.py`, `config.py`, `tests/test_planning.py`.

- [ ] Test plan validation, latest-only injection, per-turn isolation, and traced schema.
- [ ] Implement bounded `PlanState`/`UpdatePlanTool`, registry copy, and ephemeral
  state injection. No forced completion or hidden verifier feedback.
- [ ] Run full suite and matched planning-only and elision+planning pilots.
- [ ] Record overhead, successes, regressions and plan usage; commit.

## Stage 04: action support

Files: `tools/search.py`, `support.py`, `agent.py`, `config.py`, corresponding tests.

- [ ] Test bounded literal search with paths/line numbers and escaping symlinks.
- [ ] Test repeat reminder once per identical successful streak, reset on changed
  observations, and no effect on existing failure budgets.
- [ ] Test JSON/Python diagnostics on writes, no diagnostic-as-task-success, and
  unchanged operation success on a diagnostic error.
- [ ] Implement each behind its own flag. Run focused and full tests.
- [ ] Run separate pilots (never attribute a bundled result to one component); commit.

## Stage 05: synthesis and review

- [ ] Produce task-paired comparison, tokens/time per successful task, stop reasons,
  intervention activation, and explicit caveats.
- [ ] Review whole diff; fix material findings with regression tests.
- [ ] Verify branch status, untouched user file, tracked reports, and source hashes.
- [ ] Record adoption decision and remaining experiments. Keep branch for user review.
