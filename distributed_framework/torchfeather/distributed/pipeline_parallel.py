...
# use torch.distributed to generate csv like schedule that claims each rank what to do per timestamp
...
def pipeline_llm(
    ...
    module_names_per_stage = job_config.parallelism.module_fqns_per_model_part # fqns is short from full qualified names
...
def build_pipeline_schedule(
    ...
    # ----- My understanding ----- 
    # each "batch" generated under datasets/hf_dataest.py contains multi samples/docs until reaches max_seq_len as one batch  
    # local_batch_size is how many batches run under one pipeline and microbatch_size is further chunked each local batch size to make GPUs busy
    # Stage: sub model architecture that is assigned to a rank/device, each rank may contain multi stages. E.g. for a V-shape - dev0: stage0 (layer0, 1), stage4 (layer14, 15); dev3: stage3 (layer6, 7), stage7(layer8, 9);
...
def pipeline_module_split(
    ...
    for stage_idx in stage_indices:
        ...
        # chunk the entire model to the submodules on the module names under that stage
