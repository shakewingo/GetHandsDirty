...
def apply_ddp(
    ...
    if enable_compile:
        # pytorch compile will look at what operations can be fused as one operation in shared memory (GPU core) to reduce HBM accesses and improve performance. 
...
def apply_fsdp(
    ...
    ) # which precision to use when doing scatter-reduce 
    ...
    if tok_embeddings is not None:
        # shard the embedding layers based on dp_mesh: [dp_replicate, dp_shard]
        fully_shard(
            tok_embeddings,
            **fsdp_config,
    ...
    # TODO: don't understand - want to shard or not for the last layer?
    ...
    # forward
    transformer_blocks: list[Any] = layers
    ...
    for transformer_block, next_transformer_block in zip(
        ...
        if next_transformer_block is not None:
            if next_transformer_block.moe_enabled:
                ...
                ) # Note: here defines a prefetching strategy to FSDP that whenever launching the transformer block, also prefetch the  moe experts part
            else:
                transformer_block.set_modules_to_forward_prefetch(
    ...
    # backward
    reversed_transformer_blocks: list[Any] = list(reversed(layers))
...
def disable_fsdp_gradient_division(model: nn.Module) -> None:
    """
    Background: By default, pytorch will sum gradients across all ranks and then divide by the world size. This is not always desired, especially when using gradient accumulation or when the loss is already normalized by the number of tokens in the batch. 
    Similarly, for each rank, the per token loss is already normalized by the number of tokens in the batch
    ...
    Later on, in train.py() forward_backward_step(), we will manually scale the loss by the # of    global_valid_tokens.  to ensure correct gradient scaling across all ranks.
    """  
