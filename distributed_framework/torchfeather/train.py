...
class Trainer(Stateful):
    ...
    def __init__(self, job_config: JobConfig):
        ...
        # create the device mesh so you can use it to coordinate which group the current device belongs to for pp, dp, tp etc.
        ...
        if parallel_dims.dp_enabled:
            ...
            dp_degree, dp_rank = batch_mesh.size(), batch_mesh.get_local_rank() # how many ranks I should talk to which one I am
        else:
        ...
        # only load data that belongs to current dp_rank, so later on batch_generator() only extracts batch one by one from this
        ...
        ) # how many steps to accumulate gradients before optimizer step
    ...
    def batch_generator(
        ...
        while True:
            ...
            # Move tensors to the appropriate device, e.g. from CPU to GPU
    ...
    def train_step(
        ...
        # The PP schedule runs backward on the raw sum loss (including padding tokens loss?); non-PP divides by global_valid_tokens before backward.
        if parallel_dims.pp_enabled:
            for m in self.model_parts:
