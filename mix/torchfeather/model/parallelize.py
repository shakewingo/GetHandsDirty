...
def apply_non_moe_tp(
    ...
    parallelize_module(
        ...
        tp_mesh,
        { 
            # 1. row-wise parallel on embedding weights, e.g. rank0: [vocab_size/2, hidden_dim], rank1: [vocab_size/2, hidden_dim];
            # 2. partial embeddings are summed on every rank to have full (B, S, D) result via all-reduce
            # 3. output is sharded based on seq_len, which is dim 1, so using Shard(1)
            
            "token_embeddings": RowwiseParallel(
            ...
            #  This currently supports ``nn.LayerNorm``, ``nn.Dropout``, and the ``RMSNorm``
            "layernorm": SequenceParallel(), 
            
            # 1. col-wise parallel on output weights, e.g. rank0: [hidden_dim, vocab_size/2], rank1: [hidden_dim, vocab_size/2]
            # 2. split logits on vocab_size if using pytorch's built-in loss_parallel to guarantee each rank can communicate with each other when computing loss (softmax + cross entropy).
            # Specifically, loss_parallel consideres using all-reduce ops to get the max logits and sum during softmax computation.
    ...
    for transformer_block in model.layers:
        layer_plan: dict[str, ParallelStyle] = {
            ...
            # NOTE: no `inner_attention` plan entry — Attention.forward does the score
            # math with raw einsums, so sharding wq/wkv_b/wo above is what parallelizes MLA.
        ...
        if not transformer_block.moe_enabled:
            layer_plan.update(
                {
                    # The input of this comes from `post_attention_layernorm`, which is SP.
